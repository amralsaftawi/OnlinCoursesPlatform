...
public class AddLessonViewModel : IValidatableObject
{
    ...
    public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        if (Type == LessonType.Article)
        {
            if (ArticleFile == null)
            {
                yield return new ValidationResult("Please upload a `.txt` file for the article lesson.", [nameof(ArticleFile)]);
                yield break;
            }

            if (!string.Equals(Path.GetExtension(ArticleFile.FileName), ".txt", StringComparison.OrdinalIgnoreCase))
            {
        ...
        {
            if (PdfFile == null && string.IsNullOrWhiteSpace(ContentUrl))
            {
                yield return new ValidationResult("Please upload a PDF file or paste a direct PDF URL.", [nameof(PdfFile), nameof(ContentUrl)]);
                yield break;
            }

            if (PdfFile != null && !string.Equals(Path.GetExtension(PdfFile.FileName), ".pdf", StringComparison.OrdinalIgnoreCase))
            {
            ...
            if (!string.IsNullOrWhiteSpace(ContentUrl))
            {
                ...
                {
                    yield return new ValidationResult("Please enter a valid absolute URL for the PDF lesson.", [nameof(ContentUrl)]);
                }
                else if (!ContentUrl.Split('?')[0].Split('#')[0].EndsWith(".pdf", StringComparison.OrdinalIgnoreCase))
                {
                    yield return new ValidationResult("The PDF lesson URL must point directly to a `.pdf` file.", [nameof(ContentUrl)]);
                }
            }
